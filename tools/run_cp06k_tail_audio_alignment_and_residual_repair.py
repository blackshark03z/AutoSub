...
ENGLISH_FONT_SIZE = 46
PLATE_HORIZONTAL_PADDING = 16
PLATE_VERTICAL_PADDING = 8
MAX_TEXT_SAFE_WIDTH_RATIO = 0.80
CP06G_FONT_SIZE = 40
...
def main() -> None:
    ...
    evidence_dir = settings.root / "evidence" / "CP06K"
    ...
    before_contact = evidence_dir / "before_cp06i_same_timestamps.jpg"
    after_contact = evidence_dir / "after_cp06k_same_timestamps.jpg"
    ...
    render_contact_sheet(
        project_dir / "renders" / "cp06i_temporally_stable_subtitle_vertical_slice_720p.mp4",
        before_contact,
        None,
    ...
    ass_path = project_dir / "subtitles" / "cp06k_tail_audio_alignment_english.ass"
    subtitle_layout = write_source_zone_replacement_ass(timeline, intervals, ass_path)
    output_path = project_dir / "renders" / "cp06k_tail_audio_alignment_and_residual_repair_720p.mp4"
    audio_rebuild = rebuild_aligned_tts_mix(settings.root, project_id, timeline, project_dir / "audio" / "cp06k_aligned_tts_mix.wav")
    tts_mix = Path(audio_rebuild["mix_path"])
    render_preview(settings.source_path, tts_mix, ass_path, output_path, render_intervals, subtitle_layout["events"])
    ...
    render_truth = render_truth_audit(settings.source_path, output_path, render_intervals, timeline, evidence_dir)
    ...
    coverage = {
        "status": "SUPERSEDED_BY_RENDER_TRUTH_AND_TRANSITION_AUDITS",
        "reason": "CP06K acceptance requires direct final-render pixel comparison at source-matched timestamps.",
    }
    ...
    summary = {
        ...
        "subtitle_plate_layout": subtitle_layout,
        "subtitle_plate_stats": subtitle_plate_stats(subtitle_layout["events"]),
        "local_residual_repair": residual_repair,
        "audio_alignment_rebuild": audio_rebuild,
        ...
        "contact_sheets": {
            "before_cp06i_same_timestamps": str(before_contact),
            "source_eraser_mask": str(source_contact),
            "source_matched_geometry": str(source_match_contact),
            "after_cp06k_same_timestamps": str(after_contact),
            "english_layout": str(layout_contact),
            "transition_strips": strips,
        },
        "audio_rebuilt_from_cached_tts": str(tts_mix),
        "audio_rebuilt_sha256": sha256_file(tts_mix),
    ...
    (evidence_dir / "subtitle_plate_layout.json").write_text(json.dumps(subtitle_layout, indent=2), encoding="utf-8")
    (evidence_dir / "audio_alignment_qa.json").write_text(json.dumps(audio_rebuild, indent=2), encoding="utf-8")
    (evidence_dir / "local_residual_repair.json").write_text(json.dumps(residual_repair, indent=2), encoding="utf-8")
...
def rebuild_aligned_tts_mix(root: Path, project_id: str, timeline: dict, output_path: Path) -> dict:
    generation_paths = active_generation_paths(root, project_id)
    sample_rate = 48000
    ...
    for unit in timeline.get("tts_units", []):
        ...
        artifact = generation_paths.get((unit["id"], generation_id))
        if not artifact or not artifact.exists():
            missing.append({"tts_unit_id": unit["id"], "generation_id": generation_id, "reason": "active_cached_artifact_missing"})
            continue
        audio, source_rate = read_wav_mono(artifact)
        if source_rate != sample_rate:
            audio = resample_linear(audio, source_rate, sample_rate)
        member_segments = [segments[segment_id] for segment_id in unit.get("segment_ids", []) if segment_id in segments]
        utterances, detection = detect_utterance_slices(audio, sample_rate, len(member_segments))
        if len(utterances) < len(member_segments):
            if unit["id"] == "ttsu_312c98e4e76e":
                raise RuntimeError("BLOCKED_MISSING_CACHED_TTS: final grouped cached artifact does not expose every required tail utterance")
            missing.append(
                {
                    "tts_unit_id": unit["id"],
                    "generation_id": generation_id,
                    "reason": "utterance_count_below_segment_count",
                    "expected": len(member_segments),
                    "detected": len(utterances),
                }
            )
            continue
        unit_audit = {
            ...
            "artifact_duration_s": round(len(audio) / sample_rate, 3),
            "expected_spoken_segments": len(member_segments),
            "detected_utterances": len(utterances),
            "detection": detection,
            "placements": [],
        }
        for segment, utterance in zip(member_segments, utterances):
            start_sample = int(segment["start_ms"] * sample_rate / 1000)
            segment_end_sample = int(segment["end_ms"] * sample_rate / 1000)
            next_segment_start = next_member_start_sample(member_segments, segment["id"], sample_rate)
            max_end_sample = min(total_samples, segment_end_sample, next_segment_start)
            slice_audio = audio[utterance["start_sample"] : utterance["end_sample"]].copy()
            allowed = max(1, max_end_sample - start_sample)
            fitted = False
            if len(slice_audio) > allowed:
                slice_audio = resample_to_length(slice_audio, allowed)
                fitted = True
            end_sample = min(total_samples, start_sample + len(slice_audio))
            if np.any(np.abs(mix[start_sample:end_sample]) > 1e-5):
                overlap_count += 1
            mix[start_sample:end_sample] += fade_edges(slice_audio[: end_sample - start_sample], sample_rate)
            placement = {
                "segment_id": segment["id"],
                "text": subtitle_text(segment),
                "scheduled_start_s": round(start_sample / sample_rate, 3),
                "scheduled_end_s": round(end_sample / sample_rate, 3),
                "subtitle_start_s": round(segment["start_ms"] / 1000, 3),
                "subtitle_end_s": round(segment["end_ms"] / 1000, 3),
                "source_utterance_start_s": round(utterance["start_sample"] / sample_rate, 3),
                "source_utterance_end_s": round(utterance["end_sample"] / sample_rate, 3),
                "local_time_fit_applied": fitted,
            }
            unit_audit["placements"].append(placement)
            placements.append(placement)
        unit_audits.append(unit_audit)
    ...
    expected_segments = [segment for unit in timeline.get("tts_units", []) for segment in unit.get("segment_ids", [])]
    placed_segments = [placement["segment_id"] for placement in placements]
    omission_count = len(set(expected_segments) - set(placed_segments))
    duplication_count = len(placed_segments) - len(set(placed_segments))
    tail = [item for item in placements if item["subtitle_start_s"] >= 64.0]
    return {
        "mix_path": str(output_path),
        "mix_sha256": sha256_file(output_path),
        "source": "cached_grouped_tts_split_to_canonical_spoken_segment_offsets",
        "provider_calls": {"gemini": 0, "elevenlabs": 0},
        "unit_audits": unit_audits,
        "tail_placements": tail,
        "who_made_this_crazy_game": next((item for item in placements if item["text"] == "Who made this crazy game?"), None),
        "missing": missing,
        "overlap_count": overlap_count,
        "omission_count": omission_count,
        "duplication_count": duplication_count,
        "status": "PASS" if not missing and overlap_count == 0 and omission_count == 0 and duplication_count == 0 else "FAIL",
    }


def active_generation_paths(root: Path, project_id: str) -> dict[tuple[str, str], Path]:
    db_path = root / "data" / "app.db"
    con = sqlite3.connect(db_path)
    con.row_factory = sqlite3.Row
    rows = con.execute(
        """
        select segment_id, generation_id, artifact_path
        from tts_generations
        where project_id = ? and segment_id like 'ttsu_%' and status = 'ready'
        order by id
        """,
        (project_id,),
    ).fetchall()
    con.close()
    return {(row["segment_id"], row["generation_id"]): Path(row["artifact_path"]) for row in rows}
...
def detect_utterance_slices(audio: np.ndarray, sample_rate: int, expected_count: int) -> tuple[list[dict], dict]:
    candidates = []
    for threshold_ratio in [0.004, 0.006, 0.008, 0.012, 0.015, 0.018, 0.022, 0.03, 0.04, 0.05]:
        for merge_gap_s in [0.04, 0.06, 0.08, 0.10, 0.12, 0.16, 0.20, 0.24]:
            intervals = energy_intervals(audio, sample_rate, threshold_ratio, merge_gap_s)
            adjusted = merge_to_expected_count(intervals, expected_count) if len(intervals) > expected_count else intervals
            candidates.append((abs(len(adjusted) - expected_count), -len(intervals), threshold_ratio, merge_gap_s, adjusted))
            if len(adjusted) == expected_count and len(intervals) > expected_count:
                return adjusted, {
                    "threshold_ratio": threshold_ratio,
                    "merge_gap_s": merge_gap_s,
                    "mode": "over_detected_then_merged_to_expected_count",
                    "intervals_s": intervals_for_json(adjusted, sample_rate),
                }
            if len(intervals) == expected_count:
                return intervals, {
                    "threshold_ratio": threshold_ratio,
                    "merge_gap_s": merge_gap_s,
                    "mode": "exact_expected_count",
                    "intervals_s": intervals_for_json(intervals, sample_rate),
                }
    _, _, ratio, gap, intervals = sorted(candidates, key=lambda item: (item[0], item[1]))[0]
    if len(intervals) > expected_count:
        intervals = merge_to_expected_count(intervals, expected_count)
    return intervals, {
        "threshold_ratio": ratio,
        "merge_gap_s": gap,
        "mode": "nearest_count_with_merge" if len(intervals) == expected_count else "nearest_count",
        "intervals_s": intervals_for_json(intervals, sample_rate),
    }


def energy_intervals(audio: np.ndarray, sample_rate: int, threshold_ratio: float, merge_gap_s: float) -> list[dict]:
    frame = int(sample_rate * 0.02)
    if len(audio) < frame:
        return []
    rms = np.array([np.sqrt(np.mean(audio[index : index + frame] ** 2)) for index in range(0, len(audio) - frame + 1, frame)])
    threshold = max(float(np.percentile(rms, 20)) * 3.0, float(np.max(rms)) * threshold_ratio, 0.0015)
    ...
    for index, is_active in enumerate(active):
        ...
        if (not is_active or index == len(active) - 1) and start is not None:
            ...
            if (end - start) * 0.02 >= 0.06:
                raw.append({"start_sample": start * frame, "end_sample": min(len(audio), end * frame)})
            start = None
    merged = []
    merge_gap_samples = int(merge_gap_s * sample_rate)
    for interval in raw:
        if merged and interval["start_sample"] - merged[-1]["end_sample"] <= merge_gap_samples:
            merged[-1]["end_sample"] = interval["end_sample"]
        else:
            merged.append(interval)
    return merged


def merge_to_expected_count(intervals: list[dict], expected_count: int) -> list[dict]:
    intervals = [dict(item) for item in intervals]
    while len(intervals) > expected_count:
        durations = [item["end_sample"] - item["start_sample"] for item in intervals]
        ...
        if index == 0:
            ...
            previous_gap = intervals[index]["start_sample"] - intervals[index - 1]["end_sample"]
            next_gap = intervals[index + 1]["start_sample"] - intervals[index]["end_sample"]
            merge_index = index - 1 if previous_gap <= next_gap else index
        intervals[merge_index]["end_sample"] = intervals[merge_index + 1]["end_sample"]
        del intervals[merge_index + 1]
    return intervals


def next_member_start_sample(member_segments: list[dict], segment_id: str, sample_rate: int) -> int:
    for index, segment in enumerate(member_segments):
        if segment["id"] != segment_id:
            continue
        if index + 1 < len(member_segments):
            return int(member_segments[index + 1]["start_ms"] * sample_rate / 1000)
        return int(DURATION_SECONDS * sample_rate)
    return int(DURATION_SECONDS * sample_rate)


def intervals_for_json(intervals: list[dict], sample_rate: int) -> list[dict]:
    return [
        {
            "start_s": round(item["start_sample"] / sample_rate, 3),
            "end_s": round(item["end_sample"] / sample_rate, 3),
            "duration_s": round((item["end_sample"] - item["start_sample"]) / sample_rate, 3),
        }
...
def resample_linear(audio: np.ndarray, source_rate: int, target_rate: int) -> np.ndarray:
    ...
    return resample_to_length(audio, target_len)
...
def fade_edges(audio: np.ndarray, sample_rate: int) -> np.ndarray:
    if len(audio) == 0:
        return audio
    result = audio.copy()
    fade = min(int(sample_rate * 0.01), len(result) // 2)
    if fade > 1:
...
def build_local_residual_repair(source_path: Path, intervals: list[dict]) -> dict:
    cap = cv2.VideoCapture(str(source_path))
    if not cap.isOpened():
        raise RuntimeError(f"Could not open source video: {source_path}")
    boxes = []
    samples = [round(time_s, 3) for time_s in np.arange(59.40, 60.101, 1 / FPS)]
    for time_s in samples:
        frame = read_frame(cap, float(time_s))
        bbox, meta = detect_source_subtitle_bbox(frame)
        if bbox:
            boxes.append({**bbox, "time_s": float(time_s), "meta": meta})
    cap.release()
    union = union_boxes(boxes)
    if not union:
        return {"status": "NO_SOURCE_GLYPH_DETECTED", "samples": samples, "intervals": []}
    repair = {
        "segment_id": "cp06k_local_residual_59_4_60_1",
        "start_time": 59.30,
        "end_time": 60.20,
        "x": max(0, union["left_x"] - 10),
        "y": max(0, union["top_y"] - 8),
        "width": min(WIDTH, union["right_x"] - union["left_x"] + 1 + 20),
        "height": min(HEIGHT, union["bottom_y"] - union["top_y"] + 1 + 16),
        "line_count": infer_line_count(union),
        "padding": {"vertical": 8, "horizontal": 10},
        "source_bbox": union,
        "source_only": True,
        "temporal_role": "cp06k_local_residual_repair",
        "reason": "human_review_residual_thin_white_source_stroke_59_4_60_1",
    }
    return {
        "status": "APPLIED",
        "samples": samples,
...
def write_source_zone_replacement_ass(timeline: dict, intervals: list[dict], path: Path) -> dict:
    ...
    for segment in timeline["segments"]:
        ...
        if interval is None:
            continue
        layout = english_layout_for_interval(interval, text, font)
        layout["start_time"] = round(segment["start_ms"] / 1000, 3)
        layout["end_time"] = round(segment["end_ms"] / 1000, 3)
        layouts.append(layout)
        ass_text = ass_escape(layout["render_text"])
        ...
        events.append(
            ...
            f"Default,,0,0,0,,{text_override}{ass_text}"
        )
    header = """[Script Info]
ScriptType: v4.00+
PlayResX: 1280
...
"""
    ...
    return {
        ...
        "font": "Arial",
        "plate_padding": {"horizontal": PLATE_HORIZONTAL_PADDING, "vertical": PLATE_VERTICAL_PADDING},
        "plate_opacity_percent": 85,
        "events": layouts,
    }


def english_layout_for_interval(interval: dict, text: str = "", font: ImageFont.FreeTypeFont | None = None) -> dict:
    bbox = interval["source_bbox"]
    if font is None:
        font = ImageFont.truetype(str(get_settings().font_path), ENGLISH_FONT_SIZE)
    center_x = round((bbox["left_x"] + bbox["right_x"]) / 2)
    max_text_width = round((WIDTH - 120) * MAX_TEXT_SAFE_WIDTH_RATIO)
    render_text = wrap_subtitle_text(text, font, max_text_width) if text else ""
    ...
    plate_width = text_box["width"] + PLATE_HORIZONTAL_PADDING * 2
    plate_height = text_box["height"] + PLATE_VERTICAL_PADDING * 2
    plate = {
        "x": max(0, round(center_x - plate_width / 2)),
        "y": max(0, round(anchor_y - plate_height / 2)),
        "width": round(plate_width),
        "height": round(plate_height),
    }
    plate["right_x"] = min(WIDTH - 1, plate["x"] + plate["width"] - 1)
    plate["bottom_y"] = min(HEIGHT - 1, plate["y"] + plate["height"] - 1)
    clipping = plate["x"] <= 0 or plate["right_x"] >= WIDTH - 1 or plate["y"] <= 0 or plate["bottom_y"] >= HEIGHT - 1
    return {
        ...
        "font_size": ENGLISH_FONT_SIZE,
        "start_time": interval["base_start_time"] if "base_start_time" in interval else interval["start_time"],
        "end_time": interval["base_end_time"] if "base_end_time" in interval else interval["end_time"],
        "render_text": render_text,
        "line_count": line_count,
        "text_width": text_box["width"],
        "text_height": text_box["height"],
        "plate": plate,
        "plate_area_frame_percent": round(plate["width"] * plate["height"] * 100 / (WIDTH * HEIGHT), 4),
        "clipping": clipping,
    }


def wrap_subtitle_text(text: str, font: ImageFont.FreeTypeFont, max_width: int) -> str:
    if estimate_text_width(text, font) <= max_width:
        return text
    words = text.split()
    if len(words) <= 1:
        return text
...
def multiline_text_box(text: str, font: ImageFont.FreeTypeFont) -> dict:
    if not text:
        return {"width": 0, "height": 0}
    lines = text.split("\n")
    widths = [estimate_text_width(line, font) for line in lines]
    line_metrics = [font.getbbox(line or " ") for line in lines]
    heights = [bbox[3] - bbox[1] for bbox in line_metrics]
    line_gap = 6 if len(lines) > 1 else 0
    return {
        "width": max(widths) if widths else 0,
        "height": sum(heights) + line_gap * (len(lines) - 1),
    }


def subtitle_plate_stats(layouts: list[dict]) -> dict:
    ...
    return {
        ...
        "plate_average_frame_area_percent": round(statistics.mean(areas) * 100 / (WIDTH * HEIGHT), 4),
...
def english_layout_summary(timeline: dict, intervals: list[dict], font: ImageFont.FreeTypeFont) -> dict:
    ...
    for segment in timeline["segments"]:
        text = subtitle_text(segment)
        interval = interval_by_segment.get(segment["id"])
        if not text or not interval:
            continue
        layout = english_layout_for_interval(interval, text, render_font)
        layouts.append(layout)
        widths.append(layout["text_width"])
    anchor_ys = [item["anchor_y"] for item in layouts]
    return {
        "layer": "source_zone_replacement_plate_plus_text",
        "background_plate": "compact_black_text_bbox_plate",
        "font": "Arial",
        "old_font_size": CP06G_FONT_SIZE,
        "font_size": ENGLISH_FONT_SIZE,
        "style": "white_text_thin_black_outline_shadow",
        "alignment": "per_event_source_zone_center",
        ...
        "background_plate_stats": subtitle_plate_stats(layouts),
...
def render_truth_audit(source_path: Path, output_path: Path, intervals: list[dict], timeline: dict, evidence_dir: Path) -> dict:
    ...
    for time_s in mandatory_times:
        ...
        cv2.putText(pair, f"{time_s:.2f}s CP06J {status}", (652, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        ...
        boxes.append(
            {
                ...
                "reason": "english_subtitle_plate_and_text_overlay",
            }
        )
    return boxes
