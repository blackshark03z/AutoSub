...
def main() -> None:
    ...
    evidence_dir = settings.root / "evidence" / "CP06F"
    ...
    before_contact = evidence_dir / "before_cp06e_same_timestamps.jpg"
    after_contact = evidence_dir / "after_cp06f_same_timestamps.jpg"
    ...
    render_contact_sheet(
        project_dir / "renders" / "cp06e_dynamic_mask_vertical_slice_720p.mp4",
        before_contact,
        None,
    ...
    ass_path = project_dir / "subtitles" / "cp06f_decoupled_english.ass"
    write_decoupled_ass(timeline, ass_path)
    output_path = project_dir / "renders" / "cp06f_decoupled_mask_subtitle_vertical_slice_720p.mp4"
    ...
    summary = {
        ...
        "eraser_stats": stats,
        "english_subtitle_layout": english_layout_summary(timeline, font),
        "coverage": coverage,
        "sample_count": len(all_samples),
        "comparison_timestamps": comparison_times(timeline),
        "contact_sheets": {
            "before_cp06e_same_timestamps": str(before_contact),
            "source_eraser_mask": str(source_contact),
            "after_cp06f_same_timestamps": str(after_contact),
    ...
    (evidence_dir / "english_layout_audit.json").write_text(
        json.dumps(english_layout_summary(timeline, font), indent=2), encoding="utf-8"
    )
...
def write_decoupled_ass(timeline: dict, path: Path) -> Path:
    ...
    for segment in timeline["segments"]:
        ...
        events.append(
            ...
            f"Default,,0,0,0,,{ass_escape(text)}"
        )
    header = """[Script Info]
ScriptType: v4.00+
PlayResX: 1280
...
Style: Default,Arial,36,&H00FFFFFF,&H00FFFFFF,&H00000000,&H80000000,0,0,0,0,100,100,0,0,1,3,1,2,150,150,185,1
...
def english_layout_summary(timeline: dict, font: ImageFont.FreeTypeFont) -> dict:
    widths = [estimate_text_width(subtitle_text(segment), font) for segment in timeline["segments"] if subtitle_text(segment)]
    return {
        "layer": "separate_ass_text",
