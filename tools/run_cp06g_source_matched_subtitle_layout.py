...
def main() -> None:
    ...
    evidence_dir = settings.root / "evidence" / "CP06G"
    ...
    before_contact = evidence_dir / "before_cp06f_same_timestamps.jpg"
    after_contact = evidence_dir / "after_cp06g_same_timestamps.jpg"
    ...
    render_contact_sheet(
        project_dir / "renders" / "cp06f_decoupled_mask_subtitle_vertical_slice_720p.mp4",
        before_contact,
        None,
    ...
    ass_path = project_dir / "subtitles" / "cp06g_source_matched_english.ass"
    write_source_matched_ass(timeline, intervals, ass_path)
    output_path = project_dir / "renders" / "cp06g_source_matched_subtitle_vertical_slice_720p.mp4"
    ...
    summary = {
        ...
        "contact_sheets": {
            "before_cp06f_same_timestamps": str(before_contact),
            "source_eraser_mask": str(source_contact),
            "source_matched_geometry": str(source_match_contact),
            "after_cp06g_same_timestamps": str(after_contact),
...
Style: Default,Arial,40,&H00FFFFFF,&H00FFFFFF,&H00000000,&H80000000,0,0,0,0,100,100,0,0,1,3,1,2,60,60,100,1
...
def english_layout_for_interval(interval: dict) -> dict:
    ...
    anchor_y = max(500, min(640, bbox["top_y"] - 8))
    return {
